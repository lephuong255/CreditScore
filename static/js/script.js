...
document.addEventListener("DOMContentLoaded", function () {
  // Lấy các nút bấm và thêm sự kiện
  document
    .getElementById("loan-calculate-btn")
    .addEventListener("click", function () {
      showSection("loan-calculate");
    });

  document
    .getElementById("check-status-btn")
    .addEventListener("click", function () {
      showSection("check_status");
    });
});

function showSection(sectionId) {
  // Ẩn tất cả các phần tử form-section
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  logoutLink.addEventListener("click", function (event) {
    ...
    confirmationDiv.appendChild(dialogContent);
    ...
    document
      ...
      .addEventListener("click", function () {
        confirmationDiv.style.display = "none"; // Hide the confirmation dialog
      });
  });
});
...
function confirmLogout(event) {
  ...
  if (!confirm("Bạn có chắc chắn muốn đăng xuất không?")) {
    // Nếu người dùng chọn "Cancel", ngăn chặn hành động mặc định của nút
...
document.addEventListener("DOMContentLoaded", function () {
  function openPopup() {
    document.getElementById("loan-popup").style.display = "block";
  }

  function closePopup() {
    document.getElementById("loan-popup").style.display = "none";
  }

  // Open the popup when the "Calculate Loan" button is clicked
  document
    .getElementById("calculate-loan-btn")
    .addEventListener("click", function () {
      openPopup();
    });

  // Set the confirm link with the right URL when the popup opens
  function setConfirmLink(cccd, suggestedAmount) {
    const confirmLink = document.getElementById("confirm-link");
    confirmLink.href = `/confirm_loan?cccd=${encodeURIComponent(
      cccd
    )}&suggested_amount=${encodeURIComponent(suggestedAmount)}`;
  }
  ...
  // Example of setting up the confirm link (Replace these values with actual ones)
  const cccd = "{{ cccd }}";
  const suggestedAmount = "{{ suggested_amount }}";
  setConfirmLink(cccd, suggestedAmount);

  // Make sure to adjust the values when the popup opens in actual use case
});
  ...
  .addEventListener("change", function () {
    ...
    if (this.checked) {
      ...
      Array.prototype.forEach.call(
        document.getElementsByClassName("navbar-light"),
        function (elem) {
      ...
      Array.prototype.forEach.call(
        document.getElementsByClassName("navbar-dark"),
        function (elem) {
...
(function (document, window, index) {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (isAdvancedUpload) {
        ...
        if (identity != "newUserDataForm") {
          var missingValuesOption = document.querySelector(
            '#fileForm input[name = "missingValuesOption"]:checked'
          );
          ajaxData.append(
            missingValuesOption.getAttribute("name"),
            missingValuesOption.getAttribute("id")
          );
        }
  ...
  singleEntryForm.addEventListener("submit", function (e) {
    ...
    ajax.open(
      singleEntryForm.getAttribute("method"),
      singleEntryForm.getAttribute("action"),
      true
    );
    ajax.onload = function () {
      if (ajax.status >= 200 && ajax.status < 400) {
        ...
        if (data.status === "error") {
          ...
          btnSubmit.insertAdjacentHTML(
            "beforebegin",
            '<div class="alert alert-danger mt-2 font-weight-bold">' +
              data.error +
              "</div>"
          );
          ...
          var tableData =
            '<table class="table table-sm text-center"><thead class="thead-dark"><tr>';
          data.features.forEach(
            (element) => (tableData += "<th>" + element + "</th>")
          );
          tableData += "</tr></thead><tbody><tr>";
          data.values.forEach(
            (element) => (tableData += "<td>" + element + "</td>")
          );
          ...
          var appStatus =
            ...
            data.resType +
            ' mt-3 font-weight-bold w-100">The Entered Loan Application looks ' +
            data.category +
            "</div>";

          insightsArea.querySelector(".card-body div.card-text").innerHTML =
            tableData;
          insightsArea.querySelector(".card-body p.card-text").innerHTML =
            appStatus;

          insightsArea.querySelector(".col-12.col-lg-4 img").src =
            "data:image/png;base64," + data.imgUrl;
